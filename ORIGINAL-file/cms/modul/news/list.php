...
function pss_list(){
   ...
   # Paging Start
   $perpage = 20;
   $SQLpage = "SELECT * FROM pss_news $search_sql";
   $urlpage = "?".pss_url_def()."=".$_GET[pss_url_def()];
   $awal = pss_paging(1,$SQLpage,$perpage);
   $banyak = $perpage;
   # Paging End

   $SQL = "SELECT * FROM pss_news $search_sql
           ORDER BY date DESC
           LIMIT $awal,$banyak";
   ...
   do{
      ...
      $p = $_GET["p"];
      $id = $query[id];
      $tanggal = pss_date_format(2,str_replace("-","",$query[date]));
      ...
      $HTML .= "<td class=\"tbl_2 tbl_3\" valign=\"top\" align=\"center\">$tanggal</td>";
