<?
function pss_list(){
   global $pss_page, $pss_design_block;
   ...
   # Paging Start
   $perpage = 20;
   $SQLpage = "SELECT * FROM pss_design_block $search_sql";
   $urlpage = "?".pss_url_def()."=".$_GET[pss_url_def()];
   $awal = pss_paging(1,$SQLpage,$perpage);
   $banyak = $perpage;
   # Paging End

   $SQL = "SELECT * FROM pss_design_block $search_sql
           ORDER BY page ASC, pos ASC, name ASC
           LIMIT $awal,$banyak";
   ...
   do{
      ...
      $subject = $pss_page[$query["page"]]." / ".$pss_design_block[$query['name']];
