...
function pss_list(){
   ...
   # Paging Start
   $perpage = 20;
   $SQLpage = "SELECT * FROM pss_testimonial $search_sql";
   $urlpage = "?".pss_url_def()."=".$_GET[pss_url_def()];
   $awal = pss_paging(1,$SQLpage,$perpage);
   $banyak = $perpage;
   # Paging End

   $SQL = "SELECT * FROM pss_testimonial $search_sql
           ORDER BY id DESC
           LIMIT $awal,$banyak";
   ...
   do{
      ...
      $subject = $query[name];
      $status = $query[status] == 1 ? "<b style=\"color:green;\">Shown</b>" : "<b style=\"color:red;\">Hidden</b>";
      ...
      $HTML .= "<td class=\"tbl_2 tbl_3\" valign=\"top\" align=\"center\">$subject</td>";
      $HTML .= "</tr>";
