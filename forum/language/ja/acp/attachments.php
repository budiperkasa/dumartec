...
$lang = array_merge($lang, array(
	...
	'ACP_ORPHAN_ATTACHMENTS_EXPLAIN'	=> 'ここでは孤立ファイルを管理できます。ユーザーが投稿画面でファイルを記事に添付したにもかかわらず、肝心の記事を投稿しなかった場合、その添付ファイルは孤立します。ここではそれら孤立したファイルを削除または再添付することが可能です。投稿済みの記事に孤立したファイルを再添付するには添付先記事のIDナンバーが必要であり、このIDナンバーは自分で調べる必要があります。添付先記事のIDナンバーを孤立したファイルに割り当てればそのファイルはその記事において表示されます。',
	...
	'DEFINE_ALLOWED_IPS'			=> 'IPアドレス/ホスト名の認可',
	'DEFINE_DISALLOWED_IPS'			=> 'IPアドレス/ホスト名の否認',
	'DOWNLOAD_ADD_IPS_EXPLAIN'		=> '１行につき１つのIPアドレス/ホスト名を入力してください。IPアドレスを範囲指定する場合、数字と数字をハイフン(-)でつないでください。部分一致としてワイルドカード(*)を使用できます。',
	'DOWNLOAD_REMOVE_IPS_EXPLAIN'	=> '複数選択するにはマウスとキーボードをうまく活用してください（ShiftキーまたはCtrlキーを押しながらクリック）。',
	...
	'EXCLUDE_ENTERED_IP'			=> '“はい” を選択した場合、入力したIPアドレス/ホスト名は例外として追加されます。例外として追加されたIPアドレス/ホスト名はリストの有効範囲から除外されます。',
	...
	'NO_FILE_CAT'				=> 'なし',
	'NO_IMAGE'					=> 'なし',
	'NO_THUMBNAIL_SUPPORT'		=> 'サムネイルを表示できない環境にあります。サムネイルを表示させるには GD 拡張モジュールが利用可能であるか Imagemagick がサーバにインストールされている必要があります。',
	...
	'REMOVE_ALLOWED_IPS'		=> '<em>ダウンロードリスト [認可]</em> からIPアドレス/ホスト名を削除',
	'REMOVE_DISALLOWED_IPS'		=> '<em>ダウンロードリスト [否認]</em> からIPアドレス/ホスト名を削除',

	'SEARCH_IMAGICK'				=> 'Imagemagick を探す',
	'SECURE_ALLOW_DENY'				=> 'ダウンロードリストのタイプ',
	'SECURE_ALLOW_DENY_EXPLAIN'		=> 'セキュアダウンロードが有効な場合、認可/否認のどちらをリストとして使用するかを選択します',
	'SECURE_DOWNLOADS'				=> 'セキュアダウンロードを有効にする',
	'SECURE_DOWNLOADS_EXPLAIN'		=> 'このオプションを有効にした場合、ダウンロードできるユーザーをIPアドレス/ホスト名で指定（制限）できます',
	...
	'SPECIAL_CATEGORY_EXPLAIN'		=> 'ファイルの種類を指定してください。添付ファイルの表示に影響します。',
	...
	'UPLOADING_FILE_TO'				=> 'ファイル “%1$s” をID %2$d の記事へアップロードしています…',
