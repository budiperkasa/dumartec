...
$lang = array_merge($lang, array(
	...
	'DEFAULT_DATE_FORMAT_EXPLAIN'	=> 'PHPの <code>date</code> 関数で使われる形式と同じです',
...
// Avatar Settings
$lang = array_merge($lang, array(
	'ACP_AVATAR_SETTINGS_EXPLAIN'	=> 'アバターとは自分の分身となるキャラクター、もしくはその画像の事です。スタイルによりますが、アバター画像はトピック表示ページ内ユーザー名の真下に表示されます。ここではユーザーが自分のアバターを掲示板に載せる方法について設定できます。アバター画像をアップロードするには、アバターのアップロードディレクトリが存在しており且つアクセス権が書き込み可である必要があります。またファイルサイズの上限はアップロードされるアバター画像にのみ適用されることに注意してください。リンク経由で表示されるアバターはこの限りではありません。',
...
$lang = array_merge($lang, array(
	...
	'BOXES_MAX'					=> 'PMフォルダの最大数',
	'BOXES_MAX_EXPLAIN'			=> 'ユーザーが作成できるPMフォルダの最大数を指定してください',
	'ENABLE_PM_ICONS'			=> 'トピックアイコンの使用を許可する',
	'FULL_FOLDER_ACTION'		=> '満杯フォルダのデフォルト動作',
	'FULL_FOLDER_ACTION_EXPLAIN'=> 'フォルダが満杯状態の時の、新しい受信メッセージに対するデフォルト動作です。各ユーザーはユーザーCPでこの動作を変更できます。この設定がユーザー設定を上書きする事はありません。例外として “送信済みメッセージ” フォルダだけは必ず古いメッセージから順に削除していきます。',
	...
	'PM_MAX_RECIPIENTS_EXPLAIN'	=> 'プライベートメッセージの送信先として指定できるユーザー数の上限です。0 に設定すると上限はありません。送信者のデフォルトグループのグループ設定が優先されることに注意してください。グループ設定ページにおける “プライベートメッセージ送信先ユーザーの最大数” が 0 に設定されている場合にのみ、この設定が適用されます。',
));

// Post Settings
$lang = array_merge($lang, array(
	'ACP_POST_SETTINGS_EXPLAIN'			=> 'ここでは投稿に関する設定を行う事ができます',
	'ALLOW_POST_LINKS'					=> '投稿記事/プライベートメッセージでリンクを許可する',
	...
	'ENABLE_QUEUE_TRIGGER'			=> '一見さんチェック機能を有効にする',
	'ENABLE_QUEUE_TRIGGER_EXPLAIN'	=> '投稿数が下の “一見さん解除までの投稿数” を下回る登録ユーザー と ゲストユーザー は一見さんとみなされ、投稿記事がモデレータによってチェックされます。モデレータが記事を承認するまでフォーラムに記事が掲載されることはありません。この機能は、フォーラムパーミッションオプション “承認なしで投稿可能” を持つユーザーに対しても有効です。',
	'QUEUE_TRIGGER_POSTS'			=> '一見さん解除までの投稿数',
	'QUEUE_TRIGGER_POSTS_EXPLAIN'	=> '“一見さんチェック機能を有効にする” が “はい” に設定されている場合、この投稿数を下回る登録ユーザーは一見さんと見なされます。一見さんによる記事はモデレータが承認するまでフォーラムに掲載されません。',

	'BUMP_INTERVAL'					=> 'トピックの上げ期間',
	'BUMP_INTERVAL_EXPLAIN'			=> 'あるトピックが返信された時、そのトピックが自動的に上がり続ける期間を指定できます。期間は 最後に返信された日時からの経過時間 であることに注意してください。この期間が過ぎると、新しく返信されてもトピックが上がる事はありません。',
	...
	'FLOOD_INTERVAL_EXPLAIN'		=> 'ユーザーが投稿後に再び投稿できようになるまでの時間（秒単位）です。この設定を無視して連続投稿するにはパーミッションが必要です。',
	...
	'MAX_POST_IMG_HEIGHT_EXPLAIN'	=> '記事で使用可能な画像/Flashの高さの上限です。0 に設定すると上限はありません。',
	'MAX_POST_IMG_WIDTH'			=> '画像の横幅上限',
	'MAX_POST_IMG_WIDTH_EXPLAIN'	=> '記事で使用可能な画像/Flashの横幅の上限です。0 に設定すると上限はありません。',
	'MAX_POST_URLS'					=> 'リンクの最大数',
	'MAX_POST_URLS_EXPLAIN'			=> '記事で使用可能なURLリンクの最大数です。0 に設定すると上限はありません。',
	'POSTING'						=> '投稿',
	...
	'SMILIES_LIMIT_EXPLAIN'			=> '記事で使用可能なスマイリー数の上限です。0 に設定すると上限はありません。',
...
$lang = array_merge($lang, array(
	...
	'MAX_SIG_IMG_HEIGHT_EXPLAIN'	=> 'サインで使用可能な画像/Flashの高さの上限です。0 に設定すると上限はありません。',
	'MAX_SIG_IMG_WIDTH'				=> '画像の最大サイズ（横幅）',
	'MAX_SIG_IMG_WIDTH_EXPLAIN'		=> 'サインで使用可能な画像/Flashの横幅の上限です。0 に設定すると上限はありません。',
...
$lang = array_merge($lang, array(
	...
	'ACC_ACTIVATION_EXPLAIN'	=> '無効 ： ユーザー登録を停止します<br />なし ： 登録フォームが送信された時点でアカウントを有効化します<br />ユーザー ： 新しく登録したユーザーのメールアドレスにアカウント有効化メールを送信します<br />Admin ： Adminが手動でユーザーのアカウントを有効化する必要があります',
	'ACC_ADMIN'					=> 'Admin',
	'ACC_DISABLE'				=> '無効',
	'ACC_NONE'					=> 'なし',
	'ACC_USER'					=> 'ユーザー',
//	'ACC_USER_ADMIN'			=> 'User + Admin',
	...
	'USERNAME_LETTER_NUM'		=> 'あらゆる文字（漢字・ひらがな等）、数字のみ',
	'USERNAME_LETTER_NUM_SPACERS'	=> 'あらゆる文字（漢字・ひらがな等）、数字、スペーサーのみ',
...
$lang = array_merge($lang, array(
	...
	'CAPTCHA_GD_EXPLAIN'					=> 'より高度な CAPTCHA を描画するため、GD拡張モジュールを使用します',
	'CAPTCHA_GD_FOREGROUND_NOISE_EXPLAIN'	=> 'より複雑な GD CAPTCHA を描画するため、表面ノイズを使用します',
	'CAPTCHA_GD_X_GRID'						=> 'GD CAPTCHA 背景ノイズ x-軸',
	'CAPTCHA_GD_X_GRID_EXPLAIN'				=> 'より複雑な GD CAPTCHA を描画するには数字を小さくしてください。0 に設定するとx-軸の背景ノイズは無効化されます。',
	'CAPTCHA_GD_Y_GRID'						=> 'GD CAPTCHA 背景ノイズ y-軸',
	'CAPTCHA_GD_Y_GRID_EXPLAIN'				=> 'より複雑な GD CAPTCHA を描画するには数字を小さくしてください。0 に設定するとy-軸の背景ノイズは無効化されます。',
...
$lang = array_merge($lang, array(
	...
	'SESSION_LENGTH'			=> 'セッションの有効期限',
	'SESSION_LENGTH_EXPLAIN'	=> 'セッションが発生してからこの時間が経過すると、サーバはそのセッションを破棄します',
));

// Load Settings
$lang = array_merge($lang, array(
	'ACP_LOAD_SETTINGS_EXPLAIN'	=> 'ここではサーバの処理量を減らすため、掲示板の機能をいくつか無効にできます。ほとんどのサーバではどの機能も無効にする必要はないでしょう。しかしシェアされたホスティング環境などでは必要のない機能を無効化すると有益かもしれません。また、読み込み制限（UNIX系OSのみ可能）をかけたり、有効なセッション数を制限する事によって無効なセッションを割り当てられたユーザーに対して掲示板を自動的にオフラインに移行させたりする事もできます。',

	'CUSTOM_PROFILE_FIELDS'			=> 'カスタムプロフィールフィールド',
	'LIMIT_LOAD'					=> '読み込み制限',
	'LIMIT_LOAD_EXPLAIN'			=> 'もしサーバの１分間における読み込み平均量がこの値を超えた場合、掲示板はオフライン状態に移行します。1.0 に設定した場合、CPUプロセッサは１００％利用される事を意味します。この機能はUNIX系OSにおいてのみ利用できます。もしサーバが読み込み制限機能を使用できない時はこの値は自動的に 0 に設定されます。',
	...
	'LOAD_USER_ACTIVITY_EXPLAIN'	=> '最も多く投稿したトピック/フォーラムに対するユーザーの投稿割合データをプロフィールページとユーザーCPで表示します。１００万件以上の記事が存在する掲示板ではこの機能を無効にしておく事を勧めます。',
	'RECOMPILE_STYLES'				=> 'テンプレートファイルの再コンパイル',
	'RECOMPILE_STYLES_EXPLAIN'		=> 'スタイルを構成する各テンプレートファイルが更新されているかどうかを常にチェックし、更新されていた場合は再コンパイルします',
	'YES_ANON_READ_MARKING'			=> 'ゲストユーザーに対してもトピックの既読/未読を有効にする',
	'YES_ANON_READ_MARKING_EXPLAIN'	=> 'ゲストユーザーに対するトピックの既読/未読情報を保存します。無効に設定した場合、ゲストユーザーに対して全てのトピックが既読状態になります。',
	...
	'YES_ONLINE_EXPLAIN'			=> 'トップページ、フォーラム閲覧ページ、トピック表示ページでオンラインユーザー情報を表示します',
	'YES_ONLINE_GUESTS'				=> 'オンラインデータページでゲストユーザー表示を有効にする',
	'YES_ONLINE_GUESTS_EXPLAIN'		=> 'オンラインデータページでゲストユーザー情報を表示します',
	'YES_ONLINE_TRACK'				=> 'ユーザーオンライン/オフラインデータの表示を有効にする',
	'YES_ONLINE_TRACK_EXPLAIN'		=> 'プロフィールページとトピック表示ページでユーザーのオンラインデータを表示します',
	'YES_POST_MARKING'				=> 'トピックの投稿マークを有効にする',
	'YES_POST_MARKING_EXPLAIN'		=> 'トピックに返信（または新規投稿）したかどうかを判別するためのドット（点）マークをトピックアイコン上に表示します',
	'YES_READ_MARKING'				=> 'サーバサイドによるトピックの既読を有効にする',
	'YES_READ_MARKING_EXPLAIN'		=> 'トピックの既読/未読を cookie ではなくデータベースに保存します',
));

// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'phpBB ではユーザー認証がサポートされています。ここではユーザーがログインする際のユーザー認証をどのようにして行うかの認証方式を決定できます。デフォルトでは DB、LDAP、Apache があります。認証方式それぞれについて全項目を埋める必要はありません。関連のある項目だけ入力してください。',
	...
	'LDAP_INCORRECT_USER_PASSWORD'	=> '入力ユーザー名/パスワードを用いて LDAP サーバへ接続を試みましたが失敗しました',
	'LDAP_NO_EMAIL'					=> 'メールアドレスが存在していません',
	'LDAP_NO_IDENTITY'				=> '%s についてのログインIDが見つかりません',
	'LDAP_PASSWORD'					=> 'LDAP パスワード',
	'LDAP_PASSWORD_EXPLAIN'			=> '匿名で接続する場合は空白にしてください。匿名で接続しない場合は上記ユーザーのパスワードを入力してください。Active Directory サーバでは必須です。<b>注意:</b>このパスワードは暗号化されずにデータベースに保存されます。データベースにアクセスできる人間またはこのページを閲覧できるユーザーもこのパスワードを閲覧できることに注意してください。',
 	'LDAP_PORT'						=> 'LDAP サーバポート番号',
 	'LDAP_PORT_EXPLAIN'				=> 'お望みであれば LDAP サーバに接続するためのポート番号を指定できます。デフォルトポート番号 389 と異なるポート番号を使用している場合のみ入力してください。',
	'LDAP_SERVER'					=> 'LDAP サーバ名',
	'LDAP_SERVER_EXPLAIN'			=> 'もし LDAP を使用する場合、LDAP サーバのIPアドレスかホスト名を入力してください。あるいは ldap://hostname:port/ のように URL を入力してください。',
	'LDAP_UID'						=> 'LDAP <var>uid</var>',
	'LDAP_UID_EXPLAIN'				=> 'ログインIDの検索に使われるキー情報です。例: uid, sn など。',
...
// Server Settings
$lang = array_merge($lang, array(
	'ACP_SERVER_SETTINGS_EXPLAIN'	=> 'ここではサーバ設定とパス設定を行うことができます。ここで入力したデータが正確であることを確認してください。エラーが起こった場合、エラー内容を含むメールが管理人に送信されます。ドメインを入力する際は http:// などのプロトコルを併記してください。もしサーバが使用しているポート番号が通常と異なる場合、ポート番号を変更してください。ほとんどのケースでは 80 番ポートのままで問題ありません。',

	'ENABLE_GZIP'				=> 'GZip圧縮を有効にする',
	'ENABLE_GZIP_EXPLAIN'		=> '読み込まれたページは圧縮されてユーザーに送られます。圧縮するとネットワークトラフィックは軽減されますが、そのかわりにサーバとクライアントのCPU負荷は増大します。',
	'FORCE_SERVER_VARS'			=> 'サーバ設定 と URL設定 の強制',
	'FORCE_SERVER_VARS_EXPLAIN'	=> ' “はい” を選択すると下の サーバ設定 と URL設定 が強制的に使用されます。 “いいえ” を選択すると phpBB プログラムが自動的に割り出した設定情報を使用します。',
...
$lang = array_merge($lang, array(
	...
	'CHECK_DNSBL'					=> 'DNSブラックリストによるIPアドレスのチェック',
	'CHECK_DNSBL_EXPLAIN'			=> '“はい” にした場合、ユーザー登録と記事投稿の際に、次に挙げる DNSBL サービスとのやり取りによってユーザーのIPアドレスがチェックされます：<a href="http://spamcop.net">spamcop.net</a>, <a href="http://www.spamhaus.org">www.spamhaus.org</a>。サーバの設定によってはこのやり取りはしばらく時間がかかるかもしれません。もし時間がかかったりチェックが不正確であるようならこのチェック機能を無効にすることを勧めます。',
	'CLASS_B'						=> 'A.B',
	'CLASS_C'						=> 'A.B.C',
	'EMAIL_CHECK_MX'				=> '有効な MXレコード によるメールアドレスのチェック',
	'EMAIL_CHECK_MX_EXPLAIN'		=> '“はい” にした場合、ユーザー登録とプロフィール変更の際に入力されたメールアドレスのドメインは、有効なMXレコードを基にチェックされます',
	'FORCE_PASS_CHANGE'				=> 'パスワードの有効期間',
	'FORCE_PASS_CHANGE_EXPLAIN'		=> 'この日数が経過する度にユーザーに対してパスワードの変更を要求します。0 に設定すると変更の要求はされません。',
	'FORM_TIME_MAX'					=> 'フォーム送信の有効時間 [上限]',
	'FORM_TIME_MAX_EXPLAIN'			=> 'フォーム送信ページにおいて、ユーザーはこの時間が経過する前にフォームを送信する必要があります。-1 にするとこの機能は無効化されます。その場合でもセッションの有効期間が過ぎれば送信フォームは無効になることに注意してください。',
	...
	'FORWARDED_FOR_VALID_EXPLAIN'	=> 'クライアントのリクエストヘッダにおける <var>X_FORWARDED_FOR</var> 情報が直前のリクエストと一致する時だけセッションは持続されます。IPアドレスによるアクセス禁止も <var>X_FORWARDED_FOR</var> ヘッダ情報を基にチェックされます。',
	'IP_VALID'						=> 'セッションIP認証',
	'IP_VALID_EXPLAIN'				=> 'セッションを認証する際にユーザーのIPアドレスのどの部分までを比較チェックに利用するかを決定します。<br /><samp>全て</samp>：IPアドレスの全範囲を比較します<br /><samp>A.B.C</samp>：最初の x.x.x 部分のみ<br /><samp>A.B</samp>：最初の x.x 部分のみ<br /><samp>なし</samp>：チェックを行いません<br />IPv6の場合、<samp>A.B.C</samp> は最初の４ブロックを、<samp>A.B</samp> は最初の３ブロックを比較チェックします。',
	...
	'PASS_TYPE_SYMBOL'				=> '記号/符号どちらかを含めなければならない',
	'REF_HOST'						=> 'ホストのみ',
	...
	'TPL_ALLOW_PHP'					=> 'テンプレートファイル内でPHPコードの使用を許可する',
...
// Email Settings
$lang = array_merge($lang, array(
	'ACP_EMAIL_SETTINGS_EXPLAIN'	=> 'ここで設定された情報は phpBB がユーザーにメールを送る際に使われます。ここで指定されたメールアドレスが正しいものであることを確認してください。配信不能メールは全てこのアドレスに送られて来るでしょう。もしホストサーバが PHP のメール送信機能を提供していない場合、かわりに SMTP を使ってメールを送る必要があります。その場合、SMTP サーバのアドレスが必要となります（分からなければプロバイダに問い合わせてください）。もしサーバが認証を要求する場合、接続に必要なユーザー名とパスワードを入力してください。phpBB の SMTP 認証に関して、実装されているのは基本的な認証方式のみであり他の認証方式は現在サポートされていない事に注意してください。',
	...
	'BOARD_EMAIL_FORM_EXPLAIN'		=> 'ユーザーのメールアドレスを表示する代わりに、掲示板経由でのユーザーによるメール送信を可能にします。',
	...
	'EMAIL_FUNCTION_NAME_EXPLAIN'	=> 'PHPプログラムが使用するメール関数です',
	...
	'SMTP_PASSWORD_EXPLAIN'			=> 'SMTP サーバが要求する場合のみ入力してください',
	'SMTP_PLAIN'					=> 'PLAIN',
	'SMTP_POP_BEFORE_SMTP'			=> 'POP-BEFORE-SMTP',
	'SMTP_PORT'						=> 'SMTP サーバポート番号',
	'SMTP_PORT_EXPLAIN'				=> 'SMTP サーバポート番号が通常（２５番ポート）と異なっている場合のみ変更してください',
	'SMTP_SERVER'					=> 'SMTP サーバアドレス',
	'SMTP_SETTINGS'					=> 'SMTP 設定',
	'SMTP_USERNAME'					=> 'SMTP ユーザー名',
	'SMTP_USERNAME_EXPLAIN'			=> 'SMTP サーバが要求する場合のみ入力してください',
	'USE_SMTP'						=> 'SMTP サーバの使用',
	'USE_SMTP_EXPLAIN'				=> 'PHP メール関数を使うかわりに SMTP サーバを使いたい場合、 “はい” を選択してください',
));

// Jabber settings
$lang = array_merge($lang, array(
	'ACP_JABBER_SETTINGS_EXPLAIN'	=> 'ここでは Jabber を使用するための設定を行う事ができます。Jabber とはオープンソースプロトコル XMPP を採用したメッセージングサーバであり、誰でも利用できます。いくつかの Jabber サーバは MSN Messenger, Yahoo! Insant Messenger, IRC, ICQ などのインスタントメッセンジャのネットワークへ接続可能なゲートウェイを持っており、これらのネットワークへメッセージを送ることができます。全ての Jabber サーバが全てのプロトコルに対応しているわけではなく、そのためプロトコルの違いによってデータの転送に失敗する事もありえます。入力されたデータがすでに登録済みのアカウントデータである事を確認してください - phpBB はここで入力されたデータを基に Jabber サーバへ接続します。',

	'JAB_ENABLE'				=> 'Jabber を有効にする',
	'JAB_ENABLE_EXPLAIN'		=> 'Jabber による通知メッセージを有効にする。',
	'JAB_GTALK_NOTE'			=> '<samp>dns_get_record</samp> 関数が見つからないため、Google トーク を使用できないことに注意してください。 この関数を PHP4 で利用することはできませんし、Windows プラットフォームで実装されてもいません。さらに MacOS を含む現在の BSD ベースシステム上でも機能しません。',
