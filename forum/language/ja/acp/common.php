...
$lang = array_merge($lang, array(
	...
	'ACP_VC_CAPTCHA_DISPLAY'			=> 'CAPTCHA画像プレビュー',
	...
	'DIRECTORY_NOT_DIR'			=> '入力されたパス “%s” はディレクトリではありません',
	'DIRECTORY_NOT_WRITABLE'	=> '入力したパス “%s” のディレクトリは書き込み可ではありません',
	'DISABLE'					=> '無効にする',
	'DOWNLOAD'					=> 'ダウンロード',
...
// PHP info
$lang = array_merge($lang, array(
	'ACP_PHP_INFO_EXPLAIN'	=> 'ここではサーバにインストールされているPHPの設定情報（php.ini）を確認できます。設定情報には拡張モジュール、利用可能な変数、デフォルト設定の詳細が含まれています。これらの情報はサイトに何か問題が起こった場合、その問題を診断する時に役立つはずです。ホスティングサービスを利用している場合、サーバ運営会社によってはセキュリティ上の理由で表示される情報が制限されている可能性があります。www.phpbb.com のサポートフォーラムの<a href="http://www.phpbb.com/about/team/">オフィシャルチームメンバー</a>に質問する時以外はこのページにおけるいかなる情報も口外すべきではない事を忠告しておきます。',

	'NO_PHPINFO_AVAILABLE'	=> 'PHPの設定情報（php.ini）を取得できません。セキュリティ上の理由で phpinfo() が使えなくなっています。',
));

// Logs
$lang = array_merge($lang, array(
	'ACP_ADMIN_LOGS_EXPLAIN'	=> 'ここではAdminによって実行された全アクションのログをリストアップしています。ログはユーザー名、日時、IPアドレス、アクションでソート可能です。パーミッション次第でログの削除も可能です。',
...
$lang = array_merge($lang, array(
	...
	'ADMIN_LOG'					=> 'Adminによるアクションログ',
	'ADMIN_LOG_INDEX_EXPLAIN'	=> 'Adminによって最近実行された５つのアクションを表示しています。全ログを参照するには次のリンクをクリックしてください。',
	'AVATAR_DIR_SIZE'			=> 'アバターディレクトリのサイズ',

	'BOARD_STARTED'		=> '掲示板開始日時',
 	'BOARD_VERSION'		=> 'phpBBバージョン',
	...
	'PURGE_CACHE_EXPLAIN'	=> '掲示板のキャッシュデータを全て消去します。テンプレートファイルとSQLファイルの消去を含みます。',
	...
	'VALUE'					=> '値',
	'VIEW_ADMIN_LOG'		=> '全てのAdminログを表示',
	'VIEW_INACTIVE_USERS'	=> '全ての無効アカウントを表示',

	'WELCOME_PHPBB'			=> 'phpBBへようこそ',
...
$lang = array_merge($lang, array(
	...
	'INACTIVE_REASON_MANUAL'		=> 'Adminによってアカウントを停止された',
	...
	'INACTIVE_USERS_EXPLAIN'		=> 'アカウントが無効（停止）状態の登録ユーザーのリストです。無効アカウントに対して、有効化、削除、アカウント有効化メールの再送信を行うことができます。',
	'INACTIVE_USERS_EXPLAIN_INDEX'	=> 'アカウントが無効（停止）状態の登録ユーザーの最近１０件のリストです。 全ての無効アカウントを参照するには次のリンクをクリックしてください。移動先のページでアカウントの有効化、削除、アカウント有効化メールの再送信を行う事ができます。',
...
$lang = array_merge($lang, array(
	...
	'LOG_ACL_ADD_USER_GLOBAL_A_'		=> '<strong>Adminパーミッションを追加 or 変更しました</strong><br />≫ %s',
	'LOG_ACL_ADD_GROUP_GLOBAL_A_'		=> '<strong>Adminパーミッションを追加 or 変更しました</strong><br />≫ %s',

	'LOG_ACL_ADD_ADMIN_GLOBAL_A_'		=> '<strong>Adminパーミッションを追加 or 変更しました</strong><br />≫ %s',
	...
	'LOG_ACL_DEL_ADMIN_GLOBAL_A_'		=> '<strong>Adminパーミッションを削除しました</strong><br />≫ %s',
	...
	'LOG_FORUM_DEL_MOVE_POSTS_FORUMS'		=> '<strong>フォーラムとそのサブフォーラムを削除し、メッセージを</strong> %1$s へ</strong>移動しました</strong><br />≫ %2$s',
	...
	'LOG_IP_BROWSER_FORWARDED_CHECK'	=> '<strong>セッションIPアドレス/ブラウザ/X_FORWARDED_FOR が一致しませんでした</strong><br />≫ユーザーIPアドレス “<em>%1$s</em>” はセッションIPアドレス “<em>%2$s</em>” でチェックされ、ユーザーブラウザ “<em>%3$s</em>” はセッションブラウザ “<em>%4$s</em>” でチェックされ、ユーザーX_FORWARDED_FORは “<em>%5$s</em>” セッションX_FORWARDED_FOR “<em>%6$s</em>” でチェックされました',
	...
	'LOG_A_ROLE_ADD'		=> '<strong>Adminパーミッションセットを追加しました</strong><br />≫ %s',
	'LOG_A_ROLE_EDIT'		=> '<strong>Adminパーミッションセットを変更しました</strong><br />≫ %s',
	'LOG_A_ROLE_REMOVED'	=> '<strong>Adminパーミッションセットを削除しました</strong><br />≫ %s',
	...
	'LOG_REASON_ADDED'		=> '<strong>理由（リポート/却下）を追加しました</strong><br />≫ %s',
	'LOG_REASON_REMOVED'	=> '<strong>理由（リポート/却下）を削除しました</strong><br />≫ %s',
	'LOG_REASON_UPDATED'	=> '<strong>理由（リポート/却下）を更新しました</strong><br />≫ %s',
	...
	'LOG_UPDATE_PHPBB'		=> '<strong>phpBBをバージョン %1$s からバージョン %2$s へ更新しました</strong>',
