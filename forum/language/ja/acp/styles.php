...
$lang = array_merge($lang, array(
	...
	'ACP_STYLES_EXPLAIN'	=> 'ここではスタイルの管理を行うことができます。スタイルとは phpBB3 のデザイン（HTML、CSS、画像）を定義するファイルの集まりの事です。スタイルはテンプレートセット（HTMLファイル、JavaScriptファイル）、テーマ（CSSファイル）、イメージセット（画像ファイル）から構成されます。ここではスタイルの編集、削除、有効化/無効化、作成、インポートを行うことができます。プレビューする事によってそのスタイルがどのようなデザインなのかを実際に見て確認する事もできます。スタイル名の後ろのアスタリスク（*）は、そのスタイルが掲示板のデフォルトスタイルである事を意味しています。スタイル名の隣の数字は登録ユーザー（ボット含む）が選択しているスタイルの数です。ユーザーのスタイルをデフォルトスタイルに上書きするように設定している場合でもユーザーが選択しているスタイルがカウントされる事に注意してください。',
	'ACP_TEMPLATES_EXPLAIN'	=> 'テンプレートセットとはテンプレートファイル（HTMLファイル）と JavaScriptファイルの集まりの事です。ここではテンプレートセットの編集、削除、エクスポート、インストール等を行うことができます。テンプレートファイル内のコードを直接編集する事も可能です。',
	'ACP_THEMES_EXPLAIN'	=> 'ここではテーマの作成、インストール、編集、削除、エクスポートを行うことができます。テーマとはCSSファイルと、背景画像のような掲示板の基礎を構成する画像ファイルの集まりの事です。利用できるオプションの種類はサーバ環境設定と phpBB3 インストール時の設定に依存します。詳細を知るにはマニュアルを参照してください。もし新しいテーマを作成する場合、既存のテーマを利用するかどうかを選択できます。',
	'ADD_IMAGESET'			=> 'イメージセットの作成',
	'ADD_IMAGESET_EXPLAIN'	=> 'ここではイメージセットの作成を行うことができます。ここで利用できるオプションの種類はサーバ環境とファイルアクセス権の設定に依存します。オプションの例として、イメージセットを作成する際に既存のイメージセットを土台にできたり、イメージセットをアップロードまたは保管ディレクトリからインポートできたりします。もしイメージセットをアップロードまたはインポートする場合、イメージセット名をインポートファイル名と同じにするかどうかを選択できます（同じにする場合はイメージセット名を空白のままにしてください）。',
	'ADD_STYLE'				=> '新しいスタイルを作成する',
	'ADD_STYLE_EXPLAIN'		=> 'ここではスタイルの作成を行うことができます。ここで利用できるオプションの種類はサーバ環境とファイルアクセス権の設定に依存します。 オプションの例として、スタイルを作成する際に既存のスタイルを土台にできたり、スタイルをアップロードまたは保管ディレクトリからインポートできたりします（スタイル名は自動的に決定されます）。',
	'ADD_TEMPLATE'			=> 'テンプレートセットの作成',
	'ADD_TEMPLATE_EXPLAIN'	=> 'ここではテンプレートセットの作成を行うことができます。ここで利用できるオプションの種類はサーバ環境とファイルアクセス権の設定に依存します。オプションの例として、テンプレートセットを作成する際に既存のテンプレートセットを土台にできたり、テンプレートセットをアップロードまたは保管ディレクトリからインポートできたりします（テンプレートセット名は自動的に決定されます）。もしテンプレートセットをアップロードまたはインポートする場合、テンプレートセット名をインポートファイル名と同じにするかどうかを選択できます（同じにする場合はテンプレートセット名を空白のままにしてください）。',
	'ADD_THEME'				=> 'テーマの作成',
	'ADD_THEME_EXPLAIN'		=> 'ここではテーマの作成を行うことができます。ここで利用できるオプションの種類はサーバ環境とファイルアクセス権の設定に依存します。オプションの例として、テーマの作成に当たって既存のテーマを土台にできたり、テーマのアップロードまたは保管ディレクトリからインポートできたりします（テーマ名は自動的に決定されます）。もしテーマをアップロードまたはインポートする場合、テーマ名をインポートされるアーカイブファイル名と同じにするかどうかを選択できます（同じにする場合はテーマ名を空白のままにしてください）。',
	'ARCHIVE_FORMAT'		=> 'ファイルタイプ',
	'AUTOMATIC_EXPLAIN'		=> 'phpBB3 の自動検知プログラムに頼る場合は空白のままにしてください。',
	...
	'DELETE_IMAGESET_EXPLAIN'	=> 'ここではイメージセットのデータをデータベースから削除できます。パーミッション次第で画像ファイルを削除する事もできます。削除されたデータとファイルを元に戻す機能は用意されていないので注意してください。何かあった時のために削除する前にイメージセットをエクスポートしておく事を勧めます。',
	'DELETE_STYLE'				=> 'スタイルの削除',
	'DELETE_STYLE_EXPLAIN'		=> 'ここではスタイルの削除を行うことができます。ここでスタイルを削除しても、スタイルを構成するテンプレート、テーマ、イメージセットは削除されません。それらを削除するにはそれぞれの AdminCP ページで行う必要があります。',
	'DELETE_TEMPLATE'			=> 'テンプレートセットの削除',
	'DELETE_TEMPLATE_EXPLAIN'	=> 'ここではテンプレートセットのデータをデータベースから削除できます。パーミッション次第でテンプレートファイルを削除する事もできます。 削除されたデータとファイルを元に戻す機能は用意されていないので注意してください。 何かあった時のために削除する前にテンプレートセットをエクスポートしておく事を勧めます。',
	'DELETE_THEME'				=> 'テーマの削除',
	'DELETE_THEME_EXPLAIN'		=> 'ここではテーマのデータをデータベースから削除できます。パーミッション次第でテーマを削除する事もできます。 削除されたデータとファイルを元に戻す機能は用意されていないので注意してください。 何かあった時のために削除する前にテーマをエクスポートしておく事を勧めます。',
	'DETAILS'					=> '詳細データ',
	'DIMENSIONS_EXPLAIN'		=> ' “はい” を選択すると横幅/高さパラメータが表示されます',
	...
	'EDIT_DETAILS_TEMPLATE_EXPLAIN'		=> 'ここではテンプレートセットの詳細データを編集できます。テンプレートのデータ格納形式を変更する事もできます。テンプレートのデータ格納形式は、PHP設定とテンプレートセット内の各テンプレートファイルに対して書き込み可であるかどうかに依存します。',
	'EDIT_DETAILS_THEME'				=> 'テーマの詳細データ',
	'EDIT_DETAILS_THEME_EXPLAIN'		=> 'ここではテーマの詳細データを編集できます。 スタイルシートのデータ格納形式を変更する事もできます。スタイルシートのデータ格納形式は、PHP設定とCSSファイルに対して書き込み可であるかどうかに依存します。',
	'EDIT_IMAGESET'						=> 'イメージセットの編集',
	'EDIT_IMAGESET_EXPLAIN'				=> 'ここではイメージセットの編集を行うことができます。イメージセット内の各画像に対して画像サイズを指定する事もできます。画像サイズを指定する事によってブラウザによる画像表示におけるある種の問題を回避できます。',
	'EDIT_TEMPLATE'						=> 'テンプレートセットの編集',
	'EDIT_TEMPLATE_EXPLAIN'				=> 'ここではテンプレートセットの編集を行うことができます。変更されたテンプレートを元に戻す機能は用意されていないので注意してください。テンプレートファイルに対するファイルアクセス権（書き込み）がある場合、テンプレートファイルが直接書き換えられます。無い場合、データベースにテンプレートファイルのコード内容が格納されます。テンプレートを編集する際、 テンプレート変数 {XXXX} と条件文 &lt;!-- XXXX --&gt; をきちんと閉じるようにしてください。',
	...
	'EDIT_THEME_STORED_DB'				=> 'CSSファイルに対するアクセス権（書き込み）が無かったため、CSSコードをデータベースに格納しました',
 	'EDIT_THEME_STORE_PARSED'			=> 'このテーマはCSSファイル内にテンプレート変数（例えば {T_IMAGESET_PATH} ）を含んでいるため、ファイルシステムで運用することはできません。',
	...
	'IMAGESET_EXPORT_EXPLAIN'	=> 'ここではイメージセットのエクスポートを行うことができます。エクスポートされたイメージセットファイルは他の phpBB3 サイトにインストールできます（もちろん、この phpBB3 サイトにもインストールできます）。エクスポートするに当たって、エクスポートファイルをダウンロードするか保管フォルダ <em>store/</em> に格納するかを選択できます。保管フォルダに格納されたエクスポートファイルは後で FTP 経由でダウンロードする必要があります。 ',
	...
	'REPLACE_STYLE_EXPLAIN'		=> '削除するスタイルを使用しているユーザーが存在する場合、削除したスタイルを別のスタイルに置き換えます',
	...
	'STYLE_ERR_COPY_LONG'		=> '著作権は６０字以内である必要があります',
	...
	'STYLE_ERR_NAME_LONG'		=> 'スタイル名は３０字以内である必要があります',
	...
	'TEMPLATE_CACHE_EXPLAIN'	=> 'phpBB3 ではコンパイル済みテンプレートをデフォルトでキャッシュします。キャッシュする事によってサーバの負担とページが表示されるまでの時間を減少できます。ここでは各テンプレートファイルのキャッシュ状態を把握できます。',
	...
	'TEMPLATE_DELETED_FS'		=> 'テンプレートセットはデータベースから削除されました。ファイルは削除されずにサーバのウェブスペースにそのまま残っています。',
	...
	'TEMPLATE_EXPORT_EXPLAIN'	=> 'ここではテンプレートセットのエクスポートを行うことができます。エクスポートされたテンプレートセットファイルは他の phpBB3 サイトにインストールできます（もちろん、このphpBB3サイトにもインストールできます）。エクスポートするに当たって、エクスポートファイルをダウンロードするか保管フォルダ <em>store/</em> に格納するかを選択できます。保管フォルダに格納されたエクスポートファイルは後で FTP 経由でダウンロードする必要があります。',
	...
	'TEMPLATE_LOCATION_EXPLAIN'	=> '画像ファイルの内容がデータベースに格納される事はありません。画像はファイルシステムで保管されます。',
	...
	'THEME_EXPORT_EXPLAIN'		=> 'ここではテーマのエクスポートを行うことができます。エクスポートされたテーマファイルは他の phpBB3 サイトにインストールできます（もちろん、この phpBB3 サイトにもインストールできます）。エクスポートするに当たって、エクスポートファイルをダウンロードするか保管フォルダ <em>store/</em> に格納するかを選択できます。保管フォルダに格納されたエクスポートファイルは後でFTP経由でダウンロードする必要があります。',
	'THEME_EXPORTED'			=> 'テーマのエクスポートに成功しました。エクスポートファイルはフォルダ %s に格納されています。',
	'THEME_LOCATION'			=> 'スタイルのデータ格納形式',
	'THEME_LOCATION_EXPLAIN'	=> '画像ファイルのコード内容がデータベースに格納される事はありません。画像はファイルシステムで保管されます。',
	'THEME_NAME'				=> 'テーマ名',
