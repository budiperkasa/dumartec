...
$lang = array_merge($lang, array(
	...
	'CANNOT_EDIT_TIME'			=> '一定時間経過したため、記事を編集または削除できません',
	...
	'DISABLE_MAGIC_URL'			=> 'URLを自動的にパースしない',
	...
	'LOAD_DRAFT_EXPLAIN'		=> 'ここではあなたが投稿中に書いた下書きをロード（読み込み）できます。下書きの閲覧・編集、削除は ユーザーCP で行うことができます。',
	...
	'MAX_FLASH_HEIGHT_EXCEEDED'	=> '使用できるFlashファイルの最大サイズ（高さ）は %1$d pix です',
	'MAX_FLASH_WIDTH_EXCEEDED'	=> '使用できるFlashファイルの最大サイズ（横幅）は %1$d pix です',
	...
	'PHP_SIZE_OVERRUN'			=> '添付ファイルのサイズが大き過ぎます。アップロードできるファイルの最大サイズは %d MB です。<br />PHP設定ファイル php.ini の設定オプション upload_max_filesize の値をPHPプログラム側が上書きすることはできません。
',
	...
	'POLL_TITLE_COMP_TOO_LONG'	=> '投票のお題をページに表示すると大きくなりすぎる恐れがあります。BBCodeとスマイリーの削除を検討してください。',
	...
	'POST_CONFIRM_EXPLAIN'		=> '不正プログラムによる投稿を防ぐため、画像認証コードの入力を強制しています。認証コードは下の画像に表示されています。画像が表示されない、もしくは視覚的に問題がある場合は%s管理人%sに連絡してください。',
	...
	'POST_NORMAL'				=> '通常',
	'POST_REVIEW'				=> '記事のレビュー',
	'POST_REVIEW_EXPLAIN'		=> '記事の作成中にこのトピックに返信記事を投稿したユーザーがいます。投稿する前に全ての返信記事を確認してください。',
	...
	'STICK_TOPIC_FOR'			=> '注目/告知の有効期間',
	'STICK_TOPIC_FOR_EXPLAIN'	=> '0 か 空白にすると、注目/告知トピックが通常トピックに自動的に変更されることはありません',
	'STYLES_TIP'				=> 'Tip: テキストを選択した状態でBBCodeボタンをクリックすると選択部分がタグで囲まれます',
	...
	'TOO_MANY_URLS'				=> 'メッセージに含まれている URL が多すぎます。記事１つあたりのURLの最大数は %d です。',
	'TOO_MANY_USER_OPTIONS'		=> 'ユーザー１人あたりの投票可能オプション数を、存在している投票オプションの数以上に設定することはできません。',
	...
	'WRONG_SIZE'				=> '画像サイズ（横幅 ｘ 高さ。単位は pix ）は最小 %1$d x %2$d と最大 %3$d x %4$d の間でなければなりません。アップロードされた画像のサイズは %5$d x %6$d です。',
));
