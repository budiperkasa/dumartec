...
$lang = array_merge($lang, array(
	...
	'ACTIVATION_EMAIL_SENT_ADMIN'	=> 'アカウント有効化メールをAdminのメールアドレスへ送信しました',
	'ADD'							=> '追加',
	'ADD_BCC'						=> '追加 [BCC]',
	...
	'BOARD_DATE_FORMAT_EXPLAIN'	=> 'フォーマットはPHPの <a href="http://www.php.net/date">date()</a> 関数で使われているものと同様です',
	...
	'CANNOT_EDIT_MESSAGE_TIME'	=> '有効期限が過ぎたため、メッセージを編集/削除できません',
	...
	'CONFIRM_EXPLAIN'			=> '不正プログラムによるログインを防ぐため、ユーザー登録するには認証コードの入力が必要です。認証コードは下の画像に表示されています。画像が表示されない、もしくは視覚的に問題がある場合は%s管理人%sに連絡してください。',
	...
	'COPPA_EXPLAIN'				=> '送信ボタンをクリックすればアカウントの登録は完了しますが、両親または保護者に承諾してもらうまであなたのアカウントは有効化されません。両親または保護者のサインを記入するためのフォームを記載したメールがあなたのメールアドレスに送信されますのでそちらをご覧ください。サイン記入済み用紙の送り先もそのメールに記載されています。',
	...
	'DRAFTS_EXPLAIN'			=> 'ここではあなたがセーブした下書きのロード（読み込み）、閲覧・編集、削除を行うことができます',
	'DRAFT_UPDATED'				=> '下書きの更新に成功しました',

	'EDIT_DRAFT_EXPLAIN'		=> 'ここでは下書きの編集を行うことができます。添付ファイルと投票を下書きに含めることはできません。',
	'EMAIL_BANNED_EMAIL'		=> '入力したメールアドレスは禁止されています',
	'EMAIL_INVALID_EMAIL'		=> '入力したメールアドレスが正しくありません',
	'EMAIL_REMIND'				=> 'このフォーラムサイトであなたが登録したユーザーアカウントのメールアドレスを入力してください。ユーザーCP でメールアドレスを変更していない場合、ユーザー登録した際に入力したメールアドレスを入力してください。',
	'EMAIL_TAKEN_EMAIL'			=> '入力したメールアドレスは既に他のユーザーによって使用されています',
	'EMPTY_DRAFT'				=> '下書きをセーブするにはメッセージ本文を入力する必要があります',
	'EMPTY_DRAFT_TITLE'			=> '下書きのタイトルが入力されていません',
	'EXPORT_AS_XML'				=> 'XML形式でエクスポート',
	'EXPORT_AS_CSV'				=> 'CSV形式でエクスポート',
	'EXPORT_AS_CSV_EXCEL'		=> 'CSV形式（Excel）でエクスポート',
	'EXPORT_AS_TXT'				=> 'TXT形式でエクスポート',
	'EXPORT_AS_MSG'				=> 'MSG形式でエクスポート',
	...
	'FOES_EXPLAIN'				=> 'ユーザーをブロックリストに追加するとそのユーザーを無視できます。つまりブロックユーザーによる投稿記事はデフォルトではあなたに表示されなくなります。ブロックユーザーがあなたにプライベートメッセージを送信することは依然可能ですが、あなたはそれを自ら作成したルールによってブロックできます。モデレータと Admin をブロックリストに追加することはできないことに注意してください。',
	...
	'INVALID_CHARS_USERNAME'	=> 'ユーザ名に禁止文字が含まれています',
	...
	'LINK_REMOTE_AVATAR_EXPLAIN'=> 'リンクしたいアバター画像のURLを入力してください',
	...
	'LOGOUT_FAILED'				=> 'HTTPリクエストがセッションと一致しなかったため、ログアウトできませんでした。この症状が頻繁に起こるようなら管理人にこの事を連絡してください。',
	...
	'NO_EMAIL_USER'				=> '指定したメールアドレス/ユーザー名が見つかりませんでした',
	...
	'PASS_TYPE_ALPHA_EXPLAIN'	=> 'パスワードは %1$d 字から %2$d 字の長さにする必要があります。また大文字アルファベットと小文字アルファベット、さらに数字を含めてください。',
	'PASS_TYPE_ANY_EXPLAIN'		=> 'パスワードは %1$d 字から %2$d 字の長さにする必要があります',
	'PASS_TYPE_CASE_EXPLAIN'	=> 'パスワードは %1$d 字から %2$d 字の長さにする必要があります。また大文字アルファベットと小文字アルファベットを含めてください。',
	'PASS_TYPE_SYMBOL_EXPLAIN'	=> 'パスワードは %1$d 字から %2$d 字の長さにする必要があります。また大文字アルファベットと小文字アルファベット、さらに数字と記号を含めてください。',
	'PASSWORD'					=> 'パスワード',
	'PASSWORD_ACTIVATED'		=> '新しいパスワードは有効化されました',
	'PASSWORD_UPDATED'			=> 'あなたのメールアドレス宛てに、パスワードを記載したメールを送信しました',
	'PERMISSIONS_RESTORED'		=> 'パーミッションテストを終了しました',
	'PERMISSIONS_TRANSFERRED'	=> 'パーミッションテストを開始しました。今からあなたはユーザー <strong>%s</strong> のパーミッションセットで掲示板へアクセスすることになります。<br />Adminパーミッションをテストすることはできない点に注意してください。パーミッションテストはいつでも終了できます。',
	...
	'PM_SUBJECT'				=> 'メッセージの件名',
	'PM_TO'						=> 'メッセージ送信',
	'PM_USERS_REMOVED_NO_PM'	=> 'プライベートメッセージの受信を無効にしているため、送信リストに追加できなかったユーザーがいます',
	...
	'RULE_REMOVED_MESSAGE'				=> 'PMルールによって 1 件のプライベートメッセージを削除しました',
	'RULE_REMOVED_MESSAGES'				=> 'PMルールによって %d 件のプライベートメッセージを削除しました',
	...
	'SIGNATURE_EXPLAIN'			=> 'サインとは、投稿記事に自動的に追加されるテキストブロックのことです。最大 %d 字まで入力できます。',
	...
	'UPLOAD_AVATAR_FILE'			=> 'あなたのパソコンからアバター画像をアップロードする',
	'UPLOAD_AVATAR_URL'				=> 'URLからアバター画像をアップロードする',
	'UPLOAD_AVATAR_URL_EXPLAIN'		=> 'アバター画像のURLを入力してください。このサイトにコピーされます。',
	'USERNAME_ALPHA_ONLY_EXPLAIN'	=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります。アルファベットと数字のみ使用できます。',
	'USERNAME_ALPHA_SPACERS_EXPLAIN'=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります。アルファベット、数字、半角スペース、記号 -+_[] のみ使用できます。',
	'USERNAME_ASCII_EXPLAIN'		=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります。ASCII文字（半角英数字、限られた記号）のみ使用できます。特殊な記号は使用できません。',
	'USERNAME_LETTER_NUM_EXPLAIN'	=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります。あらゆる文字（漢字・ひらがな等）と数字のみ使用できます。特殊な記号は使用できません。',
	'USERNAME_LETTER_NUM_SPACERS_EXPLAIN'=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります。あらゆる文字（漢字・ひらがな等）、数字、半角スペース、記号 -+_[] のみ使用できます。特殊な記号は使用できません。',
	'USERNAME_CHARS_ANY_EXPLAIN'	=> 'ユーザー名は %1$d 字から %2$d 字の長さにする必要があります',
	'USERNAME_TAKEN_USERNAME'		=> '入力したユーザー名は既に使用されています。別のユーザー名を入力してください。',
	'USERNAME_DISALLOWED_USERNAME'	=> '入力したユーザー名は否認ユーザー名リストか言語フィルターリストに存在しています。別のユーザー名を入力してください。',
	'USER_NOT_FOUND_OR_INACTIVE'	=> '入力したユーザー名は登録されていないかアカウントが無効です',
	...
	'WATCHED_EXPLAIN'			=> 'ここではあなたが監視しているフォーラムとトピックを管理できます。監視中のフォーラムまたはトピックに新しい記事が投稿されるとあなたに通知メールが送信されます。監視を解除するにはフォーラムまたはトピックをマークして <em>監視を解除する</em> ボタンをクリックしてください。',
