...
* 
* @package language 
* @version $Id: styles.php,v 1.0 2007/19/04 01:12:00 acydburn Exp $
...
$lang = array_merge($lang, array( 
	'ACP_IMAGESETS_EXPLAIN'	=> 'Set de Imagens são todos os botões, fórum, folders, etc. e outros imagens não especificamente usado pelo estilo do fórum. Aqui você pode editar, exportar or deletar um Set de Imagens existente e importar or activar novos conjuntos.', 
	'ACP_STYLES_EXPLAIN'	=> 'Aqui você pode gerenciar os estilos disponíveis no seu fórum. Um estilo consiste fora de um template, tema e Set de Imagem. Você pode alterar estilos existentes, deletar, desactivar, reactivar, criar ou importar novos. Você pode também ver como ficará um estilo usando a função de prever. O estilo padrão atual está marcado com um asterisco (*). Também está listado o total de Utilizador usando o estilo, note que a substituição de estilo dos Utilizadores não é refletida aqui.',
	'ACP_TEMPLATES_EXPLAIN'	=> 'Um set template compreende todo código usado para gerar o layout do seu fórum. Aqui você pode editar um set template, deletar, exportar, importar e prever conjuntos. Você também pode modificar o código de template usado para gerar BBCode.', 
	...
	'ADD_IMAGESET_EXPLAIN'	=> 'Aqui você pode criar um novo Set de Imagens. Dependendo da configuração do seu servidor e permissões de Ficheiro. Dependendo da configuração do seu servidor e permissões dos Ficheiros você terá opções adicionais aqui. Por exemplo you pode basear este set de imagem em um existente. Você pode também enviar ou importar (da pasta de armazenamento) um Ficheiro de set de imagens. Se você enviar ou importar um Ficheiro Se você enviar ou importar um Ficheiro o nome do tema pode ser opcionalmente mantido do nome do Ficheiro (para isto deixe o nome do set de imagem em branco).', 
	'ADD_STYLE'				=> 'Criar Estilo', 
	'ADD_STYLE_EXPLAIN'		=> 'Aqui você pode criar um novo estilo. Dependendo da configuração do seu servidor e permissões dos Ficheiros você terá opções adicionais aqui. Por exemplo you pode basear este estilo em um existente. Você também pode enviar, importar (do sistema de Ficheiros) um Ficheiro de estilo. Se você enviar ou importar um Ficheiro de estilo o nome do estilo será determinado automaticamente.', 
	'ADD_TEMPLATE'			=> 'Criar Template', 
	'ADD_TEMPLATE_EXPLAIN'	=> 'Aqui você pode adicionar um novo template. Dependendo da configuração do seu servidor e permissões dos Ficheiros você terá opções adicionais aqui. Por exemplo you pode basear este set de template em um existente. Você também pode enviar, importar (do sistema de Ficheiros) um Ficheiro de template. Se você enviar ou importar um Ficheiro o nome do tema pode ser opcionalmente mantido do nome do Ficheiro (para isto deixe o nome do template em branco).', 
	'ADD_THEME'				=> 'Criar tema', 
	'ADD_THEME_EXPLAIN'		=> 'Aqui você pode adicionar um novo tema. Dependendo da configuração do seu servidor e permissões dos Ficheiros você terá opções adicionais aqui. Por exemplo você pode basear este tema em um outro existente. Você também pode enviar ou importar (da pasta de armazenamento) um Ficheiro de tema. Se você enviar ou importar um Ficheiro o nome pode ser opcional Se você enviar ou importar um Ficheiro o nome do tema pode ser opcionalmente mantido do nome do Ficheiro (para isto deixe o nome do tema em branco).', 
	'ARCHIVE_FORMAT'		=> 'Tipo do Ficheiro', 
	'AUTOMATIC_EXPLAIN'     => 'Deixe em branco para uma detecção automatica.',
	...
	'CACHE_FILENAME'				=> 'Ficheiro de Template', 
	'CACHE_FILESIZE'				=> 'Tamanho do Ficheiro', 
	'CACHE_MODIFIED'				=> 'Modificado', 
	'CONFIRM_IMAGESET_REFRESH'		=> 'Você tem certeza que deseja actualizar todas os dados de Set de Imagens? As configurações dos conjuntos de imagens irão substituidos todas as modificações que você fez fora do Editor de Set de Imagens.', 
	'CONFIRM_TEMPLATE_CLEAR_CACHE'	=> 'Você tem certeza que deseja limpar todas as versões em cache dos ficheiros do seu template ?', 
	'CONFIRM_TEMPLATE_REFRESH'		=> 'Você tem certeza que deseja actualizar todos os dados de template no base de dados com o conteúdo dos ficheiros no sistema de ficheiros? Isto substituirá todas as modificações que você fez fora do editor de template enquanto o template foi armazenado na base de dados.', 
	'CONFIRM_THEME_REFRESH'			=> 'Você tem certeza que deseja actualizar todos os dados do tema na base de dados com o conteúdo dos ficheiros no sistema de ficheiros? Isto substituirá todas as modificações que você fez fora do editor de tema enquanto o tema foi armazenado na base de dados.', 
	'COPYRIGHT'						=> 'Copyright', 
	...
	'DEACTIVATE_DEFAULT'		=> 'Você não pode desactivar o estilo padrão.', 
	'DELETE_FROM_FS'			=> 'Deletar do sistema de ficheiros', 
	'DELETE_IMAGESET'			=> 'Excluir Conjunto de Imagens', 
	'DELETE_IMAGESET_EXPLAIN'	=> 'Aqui você pode remover o Conjunto de imagens selecionado da base de dados. Adicionalmente, se você tiver permissão você pode selecionar para remover o conjunto do sistema de ficheiros. Por favor note que não é possível desfazer isto. Quando o conjunto de imagens é deletado ele vai para o bom. É recomendável que você primeiro exporte seu conjunto para possível futuro uso.', 
	...
	'DELETE_TEMPLATE_EXPLAIN'	=> 'Aqui você pode remover o template selecionado da base de dados. Adicionalmente, se você tiver permissão você pode selecionar para remover o conjunto do sistema de ficheiros. Por favor, note que não há capacidade de retorno. Quando templates são deletadas elas vão para o bom. É recomendável que você primeiro exporte seu conjunto para possível futuro uso.', 
	'DELETE_THEME'				=> 'Excluir Tema', 
	'DELETE_THEME_EXPLAIN'		=> 'Aqui você pode remover o tema selecionado da base de dados. Adicionalmente, se você tiver permissão você pode selecionar para remover o set do sistema de ficheiros. Por favor, note que não há capacidade de retorno. Quando tema são deletadas elas vão para o bom. É recomendável que você primeiro exporte seu tema para possível futuro uso.', 
	'DETAILS'					=> 'Detalhes', 
	...
	'EDIT_DETAILS_IMAGESET_EXPLAIN'		=> 'Aquio você pode editar certos detalhes do conjunto de imagens como seu nome.', 
	'EDIT_DETAILS_STYLE'				=> 'Editar estilo', 
	'EDIT_DETAILS_STYLE_EXPLAIN'		=> 'Usando o formulário abaixo você pode modificar um estilo existente. Você pode alterar a combinação da template, tema e conjunto de imagens que definem o estilo próprio. Você também pode criar um estilo padrão.', 
	'EDIT_DETAILS_TEMPLATE'				=> 'Editar detalhes da template', 
	'EDIT_DETAILS_TEMPLATE_EXPLAIN'		=> 'Aqui você pode editar certos detalhes da template, como o seu nome. Você também tem a opção de trocar o armazenamento da folha de estilos do sistema de ficheiros para a base de dados e vice versa. Esta opção depende da configuração do seu PHP e se o seu conjunto de template puder ser escrito no servidor.',
	...
	'EDIT_IMAGESET_EXPLAIN'				=> 'Aqui você pode editar as imagens individuais no qual define o conjunto de imagens. Você também pode especificar dimensões para esta imagem. Dimensões são opcionais, especificá-los pode superar determinadas edições de renderização em alguns navegadores. Não especificando-os você reduz um pouco o tamanho do registo na base de dados.', 
	'EDIT_TEMPLATE'						=> 'Editar template', 
	'EDIT_TEMPLATE_EXPLAIN'				=> 'Aqui você pode editar seu conjunto template diretamente. Por favor lembre-se que essas edições são permanentes e não podem serem desfeitas depois de enviadas. Se o PHP puder gravar ficheiros de templates em seu diretório de estilos algumas alterações aqui serão gravaras diretamente nos ficheiros. Se o PHP não puder gravar estes ficheiros serão copiados para a base de dados e todas as alterações serão refletidas apenas lá. Por favor tome cuidado quando editar seu conjunto template, lembre-se de fechar todas as variáveis de termos de substituição {XXXX} e indicações condicionais.', 
	...
	'EXPORT'							=> 'Exportar',
	'EDIT_THEME_STORE_PARSED'			=> 'O tema requer que o seu stylesheet esteja analisado gramaticalmente. Isto sé e possível se estiver armazenado na base de dados.', 

	'FOREGROUND'			=> 'Primeiro Plano', 
	'FONT_COLOUR'			=> 'Cor da Fonte', 
	'FONT_FACE'				=> 'Fonte', 
	'FONT_FACE_EXPLAIN'		=> 'Você pode especificar múltiplas fontes separadas por vírgulas. Se um Utilizador não tiver a primeira fonte instalada, a próxima fonte funcionando será escolhida.',
	...
	'IMAGESET_ADDED_DB'			=> 'Novo conjunto de Imagens adicionado a Base de Dados.', 
	...
	'IMAGESET_ERR_NAME_EXIST'	=> 'Um conjunto de Imagens com o mesmo já existe.', 
	'IMAGESET_ERR_NAME_LONG'	=> 'O nome do conjunto de Imagens não pode conter mais que 30 caracteres.', 
	'IMAGESET_ERR_NOT_IMAGESET'	=> 'O ficheiro que você especificou não contém um conjunto de Imagens válido.', 
	'IMAGESET_ERR_STYLE_NAME'	=> 'Você precisa informar um nome para este conjunto de Imagens.', 
	'IMAGESET_EXPORT'			=> 'Exportar conjunto de Imagens', 
	'IMAGESET_EXPORT_EXPLAIN'	=> 'Aqui você pode exportar um conjunto de imagens no formulário de ficheiro. Este ficheiro deve conter todas os dados necessários para instalar o conjunto de imagens em outro fórum. Você deve selecionar para baixar o ficheiro diretamente ou colocá-lo na sua pasta de armazenamento para baixar depois ou via FTP.', 
	...
	'NO_IMAGESET'				=> 'Não foi possível encontrar Set de Imagens no sistema de ficheiros.', 
	'NO_IMAGE'					=> 'Nenhuma Imagem', 
	'NO_IMAGE_ERROR'			=> 'Não foi possível encontrar a imagem no sistema de ficheiros.',
	'NO_STYLE'					=> 'Não foi possível encontrar estilo no sistema de ficheiros.', 
	'NO_TEMPLATE'				=> 'Não foi possível encontrar template no sistema de ficheiros.', 
	'NO_THEME'					=> 'Não foi possível encontrar tema no sistema de ficheiros.', 
	'NO_UNINSTALLED_IMAGESET'	=> 'Nenhum pacote de Imagens desinstalado encontrado.',
	...
	'REPLACE_IMAGESET_EXPLAIN'	=> 'Este pacote de imagens irá substituir todos os estilos que estão a usa-lo.', 
	'REPLACE_STYLE'				=> 'Substituir Estilo por', 
	'REPLACE_STYLE_EXPLAIN'		=> 'Este estilo iriá substituir os Utilizadores que estão usando-o.',
	...
	'SELECT_TEMPLATE'			=> 'Selecionar ficheiro de template', 
	'SELECT_THEME'				=> 'Selecionar ficheiro de tema',
	'SELECTED_IMAGE'			=> 'Imagem Selecionada', 
	'SELECTED_IMAGESET'			=> 'Pacote de Imagens seleccionado', 
	...
	'STYLE_ERR_NOT_STYLE'		=> 'O Ficheiro importado ou enviado contém um Ficheiro válido de estilo.', 
	'STYLE_ERR_STYLE_NAME'		=> 'Você precisa especificar um nome para este estilo.', 
	'STYLE_EXPORT'				=> 'Exportar estilo', 
	'STYLE_EXPORT_EXPLAIN'		=> 'Aqui você pode exportar um estilo no formulário de ficheiro. Um estilo não precisa conter todos os elementos mas deve conter pelo menos um. Por exemplo se você criou um novo tema e set de imagens para geralmente usado em template você poderia simplesmente exportar o tema e o set de imagens e omitir o template. Você deve selecionar para baixar o ficheiro diretamente ou colocá-lo na sua pasta de armazenamento para baixar depois ou via FTP.', 
	'STYLE_EXPORTED'			=> 'Estilo exportado com sucesso e armazenado em %s.', 
	'STYLE_IMAGESET'			=> 'Pacote de Imagens', 
	...
	'TEMPLATE_CACHE_EXPLAIN'	=> 'Por padrão o phpBB armazena em cache a versão compilada dos templates. Isto reduz a carga do servidor para cada vez que a página é vista e também reduz o tempo de geração da página. Aqui você pode ver o status do cache de cada ficheiro e deletar ficheiros individuais ou o cache todo.', 
	'TEMPLATE_CACHE_CLEARED'	=> 'Cache do Template limpo com sucesso.', 
	'TEMPLATE_CACHE_EMPTY'		=> 'Não há templates em cache.', 
	'TEMPLATE_DELETED'			=> 'Pacote de templates deletado com sucesso.', 
	...
	'TEMPLATE_ERR_CACHE_READ'	=> 'O diretório de cache usado para armazenar as versões em cache dos ficheiros de template não pode ser aberta.', 
	...
	'TEMPLATE_ERR_NOT_TEMPLATE'	=> 'O ficheiro especificado não contém um um set template válido.', 
	'TEMPLATE_ERR_STYLE_NAME'	=> 'Você precisa especificar um nome válido para esta template.',
	'TEMPLATE_EXPORT'			=> 'Exportar templates', 
	'TEMPLATE_EXPORT_EXPLAIN'	=> 'Aqui você pode exportar um set de templates no formulário de um ficheiro. Este ficheiro deve conter todos os ficheiros necessários para instalar as templates em outros fórum. Você pode selecionar para baixar o ficheiro diretamente ou colocá-lo na sua pasta de armazenamento para baixar depois ou via FTP.', 
	...
	'THEME_ADDED_DB'			=> 'Novo tema adicionado a base de dados.', 
	...
	'THEME_ERR_NOT_THEME'		=> 'O Ficheiro que você especificou não contém um tema válido.', 
	...
	'THEME_EXPORT_EXPLAIN'		=> 'Aqui você pode exportar um tema no formulário ou Ficheiro. Este aquivo deve conter todas as informações necessárias para instalar o tema em outro fórum. Você pode selecionar entre baixar o Ficheiro diretamente ou colocá-lo em sua pasta de armazenamento para baixar depois ou via FTP.', 
	...
	'THEME_UPDATED'				=> 'O Tema selecionado foi actualizado com sucesso.',
